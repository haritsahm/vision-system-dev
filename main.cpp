...
int yBoundaryValue(int x)
{

}

void findGoalPost(Mat cleanField)
{
    int prev_green = 0;
    int next_green = 0;
    int counter = 0;
    bool isgreen = true;
    vector<Point> goalCandidate;
    for(int row = 0; row < cleanField.rows; row++)
    {
        vector<Point> rowPoint;
        for(int col = 0; col < cleanField.cols; col++)
        {
            int pixel = cleanField.at<uchar>(row,col);
            ...
            if(pixel == 0 && isgreen == true)
            {
                isgreen=false;
                prev_green = col-1;
            }
            else if(pixel ==0 && isgreen == false)
            {
                counter++;
            }
            ...
            {
                isgreen = true;
                if(counter < cleanField.cols/2)
                {
                    counter = 0;
                    next_green = col-1;
                    goalCandidate.push_back(Point(col, row));
                }
...
int main()
{
    ...
    Mat image = imread("lapangan1.jpg", IMREAD_COLOR);
    //  VideoCapture cap("/home/haritsahm/gazebo.mp4",cv::CAP_FFMPEG );
    //  if(!cap.isOpened())
    //  {
    //    cout << "ERROE" << endl;
    //    return -1;
    ...
    //  while(1){
    Mat RawHSVImg, RawBGRImg, undistImage;
    //    cap >> image;
    imshow("Image ", image);
    ...
    //        imshow("Convect Hull", fieldConvectHull);
    imshow("Image ", image);
    imshow("Binary", cleanField);
